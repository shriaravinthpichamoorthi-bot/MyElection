...
for idx, table in enumerate(tables):
    ...
    if "constituency" not in " ".join(headers):
        continue
    col_map = {}
    ...
    for row in rows[best_idx + 1:]:
        ...
        result = {"id": "", "name": "", "leading_candidate": None, "leading_party": None, "trailing_candidate": None, "trailing_party": None, "margin": 0}
        ...
        if "margin" in col_map and col_map["margin"] < len(texts):
            try:
                ...
                pass
        if not result["name"] and len(texts) > 1:
            result["name"] = texts[1]
        if not result["id"] and texts[0].isdigit():
            result["id"] = texts[0]
        if result["name"]:
...
for r in results[:5]:
    print(f"  {r['name']} (#{r['id']}): {r['leading_candidate']} ({r['leading_party']}) vs {r['trailing_candidate']} ({r['trailing_party']}) - margin {r['margin']}")
