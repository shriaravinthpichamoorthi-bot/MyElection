...
import { formatNumber, formatPct, allianceColor, marginClass, slugify } from '../utils/helpers';
...
export default function DistrictDetail() {
  ...
  const avgMargin = allRecs.filter((r) => r.margin_pct != null).length
    ? (
        ...
        allRecs.filter((r) => r.margin_pct != null).length
      ).toFixed(1)
    : '—';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <div>
          <h1 className="inline-flex items-center gap-3 text-2xl font-bold text-white">
            <MapFragmentIcon geometry={districtGeometry} className="h-8 w-8 shrink-0" fill="#60a5fa" background="#111827" />
            <span>{district}</span>
          </h1>
                ...
                return (
                  <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Link to={`/constituency/${slugify(r.name)}`} className="text-blue-400 hover:text-blue-300 font-medium whitespace-nowrap">
                        {r.name}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-slate-200 whitespace-nowrap">{r.winner_name || <span className="text-slate-600">Pending</span>}</td>
                    <td className="px-4 py-3">{r.winner_party ? <PartyBadge party={r.winner_party} partyColors={partyColors} size="xs" /> : '—'}</td>
                    ...
                    <td className="px-4 py-3">
                      {r.margin_pct != null ? <span className={`font-medium ${mc.color}`}>{r.margin_pct.toFixed(1)}%</span> : '—'}
                    </td>
