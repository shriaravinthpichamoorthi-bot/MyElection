    ...
    <tr style={{ borderBottom: '1px solid #0f172a' }}>
      ...
      <td style={{ padding: '11px 12px', verticalAlign: 'middle', position: 'sticky', left: 0, background: '#0a0f1e', zIndex: 1 }}>
        <Link to={`${basePath}/${slugify(name)}`}
          style={{ fontWeight: 600, color: '#a5b4fc', textDecoration: 'none', whiteSpace: 'nowrap', transition: 'color 0.15s' }}
      ...
      <td style={{ padding: '11px 12px', verticalAlign: 'middle' }}>
        {hasLiveData ? (
          ...
          <span style={{ fontSize: 11, color: 'var(--text-subtle)', fontStyle: 'italic' }}>Awaiting results</span>
        )}
      </td>
        ...
        {leadingCandidate ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span title={leadingCandidate} style={{ color: 'var(--text-body)', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 140, display: 'block' }}>{leadingCandidate}</span>
              {leadingVotes != null && (
            ...
            {leadingParty && (
              <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 6px', borderRadius: 4, color: allianceColor, background: '#0f172a', border: `1px solid ${allianceColor}55`, whiteSpace: 'nowrap', alignSelf: 'flex-start' }}>{leadingParty}</span>
            )}
          </div>
        ) : (
      ...
      <td style={{ padding: '11px 12px', verticalAlign: 'middle' }}>
        {trailingCandidate ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span title={trailingCandidate} style={{ color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 140, display: 'block' }}>{trailingCandidate}</span>
              {trailingVotes != null && (
                <span style={{ fontSize: 11, color: '#cbd5e1', fontVariantNumeric: 'tabular-nums' }}>{trailingVotes.toLocaleString('en-IN')}</span>
              )}
            </div>
            {trailingParty && (
              <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 6px', borderRadius: 4, color: 'var(--text-subtle)', background: '#0f172a', border: '1px solid rgba(100,116,139,0.5)', whiteSpace: 'nowrap', alignSelf: 'flex-start' }}>{trailingParty}</span>
            )}
          </div>
        ) : (
          <span style={{ color: 'var(--text-subtle)' }}>—</span>
      ...
      {/* Margin */}
      <td style={{ padding: '12px 12px', verticalAlign: 'middle', textAlign: 'right', color: status === 'declared' ? '#34d399' : '#a5b4fc', fontVariantNumeric: 'tabular-nums', fontWeight: 700 }}>
        {margin > 0 ? `+${margin.toLocaleString('en-IN')}` : <span style={{ color: 'var(--text-subtle)', fontWeight: 400 }}>—</span>}
      </td>
...
function NominationsSummary({ constituencyNames, allResults, detailData, hasLiveData, allianceColors }) {
  ...
  return (
    <div className="card" style={{ padding: '16px 20px' }}>
      <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
        {tally.map(e => (
          <div key={e.name} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 13, color: 'var(--text-subtle)' }}>{e.name}</span>
...
export default function LiveDistrictDetail() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {/* Breadcrumb */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-subtle)', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14, flexWrap: 'wrap' }}>
            ...
            <div style={{ position: 'relative', flex: 1, maxWidth: 260 }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="field" style={{ width: '100%', paddingLeft: 30, height: 34 }} />
            </div>
            {search && <span style={{ fontSize: 13, color: 'var(--text-subtle)' }}>{filtered.length} of {allConstituencies.length}</span>}
            {detailLoading && <span style={{ fontSize: 12, color: 'var(--text-subtle)' }}>Loading details…</span>}
          </div>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr className="tbl-head" style={{ background: '#0d1526' }}>
                    {['Constituency', 'Status', 'Leader', 'Runner-up', 'Margin', 'Alliance'].map(h => (
                      <th key={h} style={{ textAlign: h === 'Margin' ? 'right' : 'left', whiteSpace: 'nowrap', position: h === 'Constituency' ? 'sticky' : undefined, left: h === 'Constituency' ? 0 : undefined, background: h === 'Constituency' ? '#0d1526' : undefined, zIndex: h === 'Constituency' ? 2 : undefined }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  ...
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={6} style={{ padding: '32px 0', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
