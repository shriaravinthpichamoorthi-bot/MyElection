...
function StatusBadge({ status, margin }) {
  ...
  // For any other status with a positive margin (e.g., 'leading' or intermediate states)
  if (margin > 0) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {!hasLiveData && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 9, background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.18)', marginBottom: 14 }}>
          ...
          <p style={{ fontSize: 12, color: '#818cf8', fontWeight: 600 }}>Live results pending · Showing 2026 nomination data</p>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14, alignItems: 'center' }}>
        <div style={{ position: 'relative' }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="field" style={{ paddingLeft: 30, height: 34, fontSize: 12, width: '100%', minWidth: 180, maxWidth: 280 }} />
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #1e293b', background: '#080d1a' }}>
                ...
                <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: 10, fontWeight: 600, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>Status</th>
                <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: 10, fontWeight: 600, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>{hasLiveData ? 'Leader' : 'Nominated'}</th>
                <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: 10, fontWeight: 600, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>Runner-up</th>
                <SortTh label="Margin" col="margin" activeCol={sortCol} dir={sortDir} onSort={sortToggle} />
                {hasLiveData && <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: 10, fontWeight: 600, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>Round</th>}
              </tr>
            </thead>
            <tbody>
              {sorted.length === 0 && (
                <tr>
                  <td colSpan={hasLiveData ? 7 : 6} style={{ padding: '48px 16px', textAlign: 'center', color: '#334155' }}>No constituencies match the current filters</td>
                </tr>
              )}
              {sorted.map((row, i) => {
                ...
                return (
                  <tr key={row.name} className="constituency-row"
                    ...
                    <td style={{ padding: '11px 12px', position: 'sticky', left: 0, background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.008)', zIndex: 1 }}>
                      <Link to={`${basePath}/${slugify(row.name)}`}
                        style={{ color: '#a5b4fc', textDecoration: 'none', fontWeight: 600, whiteSpace: 'nowrap', fontSize: 12, transition: 'color 0.15s' }}>
                        {row.name}
                      </Link>
                    </td>
                    ...
                    <td style={{ padding: '11px 12px', color: '#475569', whiteSpace: 'nowrap' }}>
                      <Link to={`${basePath}/district/${slugify(row.district)}`}
                        style={{ color: '#64748b', textDecoration: 'none', fontSize: 11, fontWeight: 600, transition: 'color 0.15s' }}>
                        {row.district}
                      </Link>
                    </td>
                    ...
                    <td style={{ padding: '11px 12px' }}>
                      {hasLiveData ? (
                        ...
                        <span style={{ fontSize: 10, color: '#475569', fontStyle: 'italic' }}>Awaiting</span>
                      )}
                    </td>
                    ...
                    <td style={{ padding: '11px 12px' }}>
                      {row.leaderName ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          <span title={row.leaderName} style={{ color: '#e2e8f0', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 160, display: 'block', fontSize: 12 }}>{row.leaderName}</span>
                          {row.leaderParty && (
                            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 4, color: accentColor, background: '#0f172a', border: `1px solid ${accentColor}55`, whiteSpace: 'nowrap', alignSelf: 'flex-start' }}>{row.leaderParty}</span>
                          )}
                        </div>
                    ...
                    <td style={{ padding: '11px 12px' }}>
                      {row.trailingName ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          <span title={row.trailingName} style={{ color: '#cbd5e1', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 160, display: 'block', fontSize: 12 }}>{row.trailingName}</span>
                          {row.trailingParty && (
                            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 4, color: '#64748b', background: '#0f172a', border: '1px solid rgba(100,116,139,0.5)', whiteSpace: 'nowrap', alignSelf: 'flex-start' }}>{row.trailingParty}</span>
                          )}
                        </div>
                      ) : <span style={{ color: '#334155' }}>—</span>}
                    </td>
                    ...
                    {/* Margin */}
                    <td style={{ padding: '11px 12px', textAlign: 'right', color: isDeclared ? '#34d399' : '#a5b4fc', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap', fontWeight: 700, fontSize: 12 }}>
                      {row.margin > 0 ? `+${row.margin.toLocaleString('en-IN')}` : <span style={{ color: '#334155', fontWeight: 400 }}>—</span>}
                    </td>

                    {/* Round */}
                    {hasLiveData && (
                      <td style={{ padding: '11px 12px', textAlign: 'left', color: '#64748b', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap', fontSize: 11 }}>
                        {row.round ? (
